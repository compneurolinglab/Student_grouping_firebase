class StudentFormSystem {
    ...
    async submitToFirebase(studentData) {
        try {
            ...
            if (firebaseReady) {
                ...
                const existingStudent = await window.firebaseService.checkDuplicateStudent(studentData.name, studentData.birthDate);
                ...
                const result = await window.firebaseService.submitStudent(studentData);
        ...
        modal.innerHTML = `
            <div class="duplicate-modal-content">
                <div class="duplicate-header">
                    <h3>⚠️ 检测到重复提交</h3>
                    <p>系统发现已有相同姓名和生日的学生记录</p>
                </div>
                
                <div class="duplicate-comparison">
                    <div class="existing-data">
                        <h4>📋 已有记录</h4>
                        <p><strong>姓名:</strong> ${existingData.name}</p>
                        <p><strong>生日:</strong> ${this.getMonthName(existingData.birthDate.month)} ${existingData.birthDate.day}</p>
                        <p><strong>兴趣:</strong> ${existingData.interests ? existingData.interests.join(', ') : '无'}</p>
                        <p><strong>提交时间:</strong> ${new Date(existingData.submitTime).toLocaleString()}</p>
                        ${existingData.updateCount > 0 ? `<p><strong>修改次数:</strong> ${existingData.updateCount}</p>` : ''}
                    </div>
                    
                    <div class="new-data">
                        <h4>✏️ 新提交内容</h4>
                        <p><strong>姓名:</strong> ${newData.name}</p>
                        <p><strong>生日:</strong> ${this.getMonthName(newData.birthDate.month)} ${newData.birthDate.day}</p>
                        <p><strong>兴趣:</strong> ${newData.interests ? newData.interests.join(', ') : '无'}</p>
                    </div>
                </div>
                
                <div class="duplicate-actions">
                    <button class="btn-update" onclick="studentFormSystem.handleDuplicateUpdate('${JSON.stringify(newData).replace(/'/g, "\\'")}')">
                        🔄 更新现有记录
                    </button>
                    <button class="btn-cancel" onclick="studentFormSystem.closeDuplicateModal()">
                        ❌ 取消提交
                    </button>
                </div>
                
                <div class="duplicate-note">
                    <p><small>💡 选择"更新现有记录"将覆盖之前的信息，选择"取消提交"可以修改当前表单内容。</small></p>
                </div>
            </div>
        `;
    ...
    async handleDuplicateUpdate(newDataStr) {
        try {
            ...
            const result = await window.firebaseService.submitStudent(newData, true);
            ...
            this.showStatusMessage('更新失败，请重试', 'error');
        }
    }
    ...
    showFirebaseSuccessMessage(studentData, firebaseId, isUpdate = false, updateCount = 0) {
        let message = `
            <div class="success-content">
                <h3>🎉 ${isUpdate ? '更新成功！' : 'Submission Successful!'}</h3>
                ...
                ${isUpdate ? `<p><strong>修改次数:</strong> ${updateCount}</p>` : ''}
                <p class="success-note">✅ Your information has been successfully ${isUpdate ? 'updated' : 'submitted'}!</p>
            </div>
        `;
