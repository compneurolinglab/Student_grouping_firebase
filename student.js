class StudentFormSystem {
    ...
    showFirebaseSuccessMessage(studentData, firebaseId) {
        const message = `
            <div class="success-content">
                ...
                <p><strong>生日:</strong> ${this.getMonthName(studentData.birthDate.month)} ${studentData.birthDate.day}</p>
                <p><strong>兴趣:</strong> ${studentData.interests.join(', ')}</p>
                <p class="success-note">✅ 你的信息已实时同步到云端数据库！</p>
                <p class="firebase-id" style="font-size: 0.9rem; color: #666;">🔥 Firebase ID: ${firebaseId}</p>
            </div>
        `;
    ...
    saveStudentData(studentData) {
        try {
            ...
            // 也保存当前学生的ID到localStorage，用于页面刷新后的状态恢复
    ...
    checkExistingSubmission() {
        // 检查localStorage中是否有当前学生ID对应的提交记录
    ...
    showSuccessMessage(studentData) {
        ...
        const formattedDate = submitDate.toLocaleString('zh-CN', {
            year: 'numeric',
            month: '2-digit',
            day: '2-digit',
            hour: '2-digit',
    ...
    checkGroupAssignment() {
        try {
            ...
            // Check if groups exist in localStorage (saved by teacher)
    ...
    startGroupAssignmentPolling() {
        // Check for group assignment updates every 10 seconds
        setInterval(() => {
            this.checkGroupAssignment();
        }, 10000);
    }
...
window.addEventListener('load', () => {
    setTimeout(loadDraft, 500); // 延迟加载，确保其他初始化完成
});
